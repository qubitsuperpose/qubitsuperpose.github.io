...
*   **입력:** 유니타리 양자 회로 (N 큐비트) 및 해당 회로의 고유 벡터 상태 ($\psi$).  
    ...
    - 고유 벡터 $\psi$는 이 회로에 특정 방식으로 반응하는 상태입니다.
*   **목표:** $\psi$에 해당하는 고유값 (복소 단위 원상의 값)을 근사하는 실수 $\theta$ (0 <= $\theta$ < 1)를 찾기.  
    - 고유값은 복소 평면에서 단위 원 위에 있으며, $\theta$는 이 고유값의 각도를 나타냅니다. 
    - 예를 들어, $\theta$를 알면 회로의 동작을 더 잘 이해할 수 있습니다.

### 7.3  **위상 추정 문제 해결 방법:**  
*   **준비 운동:** 위상 킥백 현상을 이용하여 $\theta$에 대한 낮은 정밀도의 근사값 얻기.  
...
*   **일반적인 위상 추정 절차:** QFT를 적용하여 고정밀도로 문제 해결.  
    - QFT를 사용하면 $\theta$의 정밀도가 기하급수적으로 향상되어 매우 정확한 고유값 추정이 가능
...
*   N 큐비트 유니타리 회로와 N 큐비트 상태 $\psi$가 주어짐 ($\psi$는 U의 고유 벡터).  
    - 이는 위상 추정의 수학적 설정을 명확히 하며, 실제 양자 컴퓨터에서 구현 가능한 형태로 정의됩니다.
*   목표는 U의 $\psi$에 상응하는 고유값을 근사하는 것. 고유값은 복소 단위 원 위에 있으며, $e^{2πiθ}$ 형태로 표현됨. 목표는 0과 1 사이의 $\theta$ 값을 추정하는 것.  
...
*   Hadamard 게이트와 측정을 통해 $\theta$에 대한 정보 획득.  
...
*   M이 증가함에 따라 $\theta$의 근사 정확도 향상 (최고 근사값의 확률은 최소 40%).  
    - 이는 위상 추정이 확률적으로 작동하며, 충분한 M 값에서 높은 성공률을 보장함을 의미

### 7.11 **Order Finding 문제**  
*   $Z_N$ 및 $Z_N*$ 정의.  
    *   $Z_N$: {0, 1, 2, ..., N-1}
    *   $Z_N*$: N과 서로소인 $Z_N$의 원소들로 구성된 **곱셈군**. 즉, $\\{ A ∈ Z_N \| gcd(A, N) = 1 \\}$
...
> 
* 유니타리 행렬은 "회전 + 위상 변화"만 수행한다.
* 이 변환은 고유벡터 방향에서는 아주 간단하게 "위상 곱하기"로 표현된다.
* 전체 공간은 이런 고유벡터들로 이루어진 정규직교 기저이다.
* 따라서 유니타리 변환은 적절한 기저에서는 **대각행렬(diagonal matrix)** 형태로 이해할 수 있다.
...
> 
(θ는 실제 각도이지만, 360°(2π)를 1로 정규화해서 0 ≤ θ < 1 로 표현)
...
### 8.4 **Grover's Algorithm**
   ...
   * Grover 연산 (G) 정의:
     ...
     * $Z_f$: 탐색 대상(즉, $f(x) = 1$인 상태의 위상을 반전시키는 **오라클**oracle)이다. 이는 타겟 상태를 식별하여 양자 간섭을 유도한다.  
     * $Z_0 = 2\|0\rangle\langle0\| - I$: 모든 큐비트가 $\|0\rangle\$인 상태의 위상만 반전시키는 연산이다. $H^{\otimes n} Z_0 H^{\otimes n}$는 중첩 상태에서 **평균 진폭에 대한 반사**를 수행하여, 타겟 상태의 진폭을 반복적으로 증폭한다.
...
### 8.5 **위상 쿼리 게이트 (Phase Query Gates)**
   ...
   * $U_F:\|x,y> → \|x, y ⊕ f(x)>$ (고전적 출력 방식).  
     * $U_F$는 함수 값을 보조 큐비트에 기록하며, 고전적 연산을 양자 회로로 확장한 형태이다.
   * $Z_F: \|x> → (-1)^f(x) \|x>$ (위상 반전 방식).  
     * $Z_F$는 출력 대신 위상 변화를 통해 함수 정보를 반영하며, 양자 간섭에 적합하다.
   ...
   * n비트 OR 함수에 대한 위상 쿼리 게이트($Z_0$)를 정의한다.  
     * $Z_0$는 모든 비제로 상태를 반전시켜 알고리즘의 기하학적 해석을 단순화한다.

### 8.6 **Grover 연산 분석**
   * n개의 큐비트로 구성된 레지스터 Q를 정의한다.  
     * 레지스터 Q는 양자 상태를 저장하며, Grover 알고리즘의 작업 공간 역할을 한다.
   * 상태 집합을 $A_0$ ($f(x) = 0$인 $x$ 집합)과 $A_1$ ($f(x) = 1$인 $x$집합, 해 집합)으로 분할한다.  
     * 이 분할은 해와 비해를 구분하여 양자 상태의 진폭을 조작하는 데 핵심적이다.
   * 균등 중첩 상태 정의: $\|u> = (1/√N) ∑\|x> = √(N-s)/N \|A_0> + √(s/N) \|A_1>$.  
     * $\|u>$는 초기 상태로, 해와 비해의 기여도를 비율로 표현하며, $s$는 해의 개수이다.
   * Grover 알고리즘의 첫 번째 단계는 $\|u>$에 $Q$를 입력하는 것이다.  
     * $Q$는 Grover 연산의 반복 단위로, 상태를 점진적으로 해 방향으로 회전시킨다.
   ...
   * $Z_F$ 게이트의 작용: $\|A_0> → \|A_0>, \|A_1> → -\|A_1>$.  
     * $Z_F$는 해 집합의 위상만 반전시켜 간섭 효과를 유도한다.
...
* **순수 상태 예시**
    ...
    예: 1큐비트의 $\|\psi\rangle = a\|0\rangle + b\|1\rangle$
    ...
    예: $\|+\rangle = \frac{1}{\sqrt{2}}(\|0\rangle + \|1\rangle$)
...
*   **혼합 상태 예시**
    ...
    예: 50% 확률로 $\|0\rangle$, 50% 확률로 $\|1\rangle$
    ...
    → 이것은 **완전히 고전적인 동전 던지기와 같은 혼합 상태**.

*   **순수 상태와 혼합 상태 차이 핵심**

    | 구분   | 순수 상태           | 혼합 상태                 |           |       |                   |           |
    | ---- | --------------- | --------------------- | --------- | ----- | ----------------- | --------- |
    | 정의   | 하나의 상태로 완전히 결정  | 여러 상태의 확률적 혼합         |           |       |                   |           |
    | 조건   | ($\rho^2 = \rho$) | ($\rho^2 \ne \rho$) 일반적 |           |       |                   |           |
    | 엔트로피 | 0               | > 0                   |           |       |                   |           |
    | 예    | (               $\| 0\rangle$), (          $\| +\rangle$) | $50\% ( \| 0\rangle) + 50\% ( \| 1\rangle$) |
...
> 밀도 행렬의 볼록 결합은 양자 상태의 확률적 또는 임의적 선택을 나타냅니다. 상태 벡터는 이러한 방식으로 평균화할 수 없습니다. \
왜냐하면 상태 벡터의 선형 결합(예: $c_1\|\psi_1\rangle + c_2\|\psi_2\rangle$)은 서로 다른 상태들을 확률적으로 섞는 것(혼합)이 아니라, 동시에 중첩하는 것(중첩)을 의미하여 물리적으로 다른 상황이 되기 때문입니다
