...
- 준비상태:
    - **$q_A$**: 텔레포트할 **정보**를 담고 있는 큐비트 (앨리스 소유)
    - **$EP_A$**: **얽힘 쌍**의 첫 번째 큐비트 (앨리스 소유) 
    - **$EP_B$**: **얽힘 쌍**의 두 번째 큐비트 (밥 소유, 장거리 전송 완료 상태)

- 양자 회로 다이어그램은 이 과정을 시각적으로 표현합니다. 회로는 다음과 같은 단계로 구성됩니다:  
    1. 앨리스가 $q_A$와 $EP_A$에 제어-NOT(CNOT) 게이트를 적용합니다. 이는 Q의 상태를 A에 부분적으로 전달
    2. 앨리스가 $q_A$에 Hadamard 게이트를 적용하여 상태를 중첩시킵니다.  
    3. 앨리스가 $EP_A$와 $q_A$를 측정하여 2비트를 얻고, 이를 밥에게 전송합니다.  
    4. 밥은 받은 2비트에 따라 $EP_B$에 X(비트 반전) 또는 Z(위상 반전) 게이트 적용하여 $q_A$의 상태를 복원

    <img src="https://www.researchgate.net/profile/Alexis-Kirke/publication/337089622/figure/fig9/AS:842531705192449@1577886620854/Circuit-diagram-of-quantum-teleportation.png" style="width:70%">
...
- 앨리스는 변환된 큐비트 A를 밥에게 보냅니다. 
    ...
    <img src="https://www.researchgate.net/profile/Walter-Pogosov/publication/329451785/figure/fig1/AS:962155608289280@1606407181000/Schematic-view-of-the-simplest-version-of-the-superdense-coding-protocol.png" style="width:60%">
...
- 양자 순간이동과 초고밀도 코딩의 비교
    - 양자 텔레포테이션 (Quantum Teleportation)
        * **전송 방향:** 앨리스 $\to$ 밥
        * **전송 내용:** 앨리스가 가진 **미지의 양자 상태** $\|\psi\rangle$ (양자 정보)
        * **프로토콜 요약:**
            1.  앨리스와 밥은 얽힘 쌍(e-bit)을 공유합니다.
            2.  앨리스는 자신의 양자 상태와 얽힘 큐비트를 결합하여 **벨 기저 측정**을 수행합니다.
            3.  측정 결과인 2비트의 **고전 정보**를 밥에게 전송합니다.
            4.  밥은 이 고전 정보를 사용하여 자신의 얽힘 큐비트에 $Z$와 $X$ 게이트를 적용하여 원래의 $\|\psi\rangle$ 상태를 **재구성**합니다.
    - 초고밀도 코딩 (Superdense Coding)
        * **전송 방향:** 앨리스 $\to$ 밥
        * **전송 내용:** 2비트의 **고전 정보** (00, 01, 10, 11)
        * **프로토콜 요약:**
            1.  앨리스와 밥은 얽힘 쌍(e-bit)을 공유합니다.
            2.  앨리스는 자신이 보내고 싶은 2비트에 따라 자신의 얽힘 큐비트에 $X$ 또는 $Z$ 게이트를 적용하여 얽힘 상태를 **네 가지 벨 상태** 중 하나로 인코딩합니다.
            3.  앨리스는 인코딩된 **단일 큐비트**만을 밥에게 전송합니다.
            4.  밥은 전송받은 큐비트와 자신이 가진 큐비트를 함께 **벨 기저 측정**하여 앨리스가 보낸 2비트를 복구합니다.
    - **결론**
        - 두 프로토콜 모두 양자 얽힘을 사용하여 통신을 수행하지만, 텔레포테이션은 **양자 상태 자체를 전송**하기 위해 고전적 보조 채널(2비트)이 필요하며, 초고밀도 코딩은 얽힘을 활용하여 **고전적 정보의 전송 효율을 두 배**로 높이는 데 사용됩니다. 즉, **양자 정보를 보낼 것인가**, 아니면 **고전 정보를 효율적으로 보낼 것인가**의 차이입니다.


### 4.7 **CHSH 게임**  

<img src="https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEi2vcIkLI3xH6SjHyAMIki2h9N8ORjBU_vSdnA9OFzWl3kMdWjD9jRz4hPGsG6Ew3bNk8Tby4h9nzUqK_U7qiVcdIJOKg-0hc4NGdmQCKmHHkispR5SQwmPuQGAKTMKE9n4BMTqTjma2gqB/s1600/cbcqcpa.png" style="width:70%">
...
- CHSH 게임 상세 보기
    ...
    1\. CHSH 게임 규칙 보기
    * **목표:** 앨리스와 밥이 출력 **$a$와 $b$**를 정하여 **$a \oplus b = x \cdot y$**를 만족시켜야 합니다.
        * $a \oplus b$: XOR 연산 (두 값이 다르면 1, 같으면 0)
        * $x \cdot y$: AND 연산 (두 값이 모두 1일 때만 1, 아니면 0)
    * **입력 ($x, y$):** 찰리가 앨리스에게 $x$ (0 또는 1)를, 밥에게 $y$ (0 또는 1)를 무작위로 보냅니다. 총 네 가지 경우($\text{00, 01, 10, 11}$)가 있으며 각각 $25\%$ 확률입니다.

    | 입력 ($x, y$) | $x \cdot y$ (목표 $a \oplus b$) |
    | :--- | :--- |
    | **0, 0** | 0 |
    | **0, 1** | 0 |
    | **1, 0** | 0 |
    ...
    2\.고전적 전략 (Classical Strategy): $\mathbf{P_{max} = 0.75}$ \
    고전적인 전략에서 앨리스와 밥은 게임 전에 **하나의 결정론적 전략**에 합의할 수 있지만, 게임 중에는 서로의 입력($x$와 $y$)을 알 수 없습니다.

    **최적의 고전 전략 (가장 단순한 전략)**
    
    앨리스와 밥은 다음과 같이 약속합니다.
    > **"무조건 $a=0$과 $b=0$으로 대답하자."**

    이 전략을 네 가지 입력 경우에 적용해 봅시다.

    | 입력 ($x, y$) | $x \cdot y$ (목표) | $a=0, b=0$ (출력) | $a \oplus b$ | 성공 여부 ($a \oplus b = x \cdot y$) |
    | :--- | :--- | :--- | :--- | :--- |
    | **0, 0** | 0 | $a=0, b=0$ | 0 | **성공 (O)** |
    | **0, 1** | 0 | $a=0, b=0$ | 0 | **성공 (O)** |
    | **1, 0** | 0 | $a=0, b=0$ | 0 | **성공 (O)** |
    | **1, 1** | 1 | $a=0, b=0$ | 0 | **실패 (X)** |

    * **승률:** 네 가지 경우 중 **세 번** 성공합니다.
    * **고전적 최대 승률:** $\frac{3}{4} = 0.75$

    어떤 복잡한 고전적 전략을 사용하더라도, 앨리스와 밥은 모든 경우를 동시에 만족시킬 수 없으며, **항상 3/4 이상의 승률을 달성할 수 없습니다.** (이는 $\text{CHSH}$ 부등식 $\|S\| \le 2$와 동치입니다.)

    3\. 양자 전략 (Quantum Strategy): $\mathbf{P_{max} \approx 0.8535}$

    양자 전략에서는 앨리스와 밥이 게임 시작 전 **최대로 얽힌 2-큐비트 상태($\|\psi\rangle$)**를 공유합니다.

    1) **양자 얽힘의 마법**

    얽힘 큐비트의 특성은 다음과 같습니다.

    1.  **공유:** 앨리스는 큐비트 $q_A$를, 밥은 $q_B$를 가지고 있습니다.
    2.  **측정:** 두 사람은 자신의 큐비트에 대해 **입력 ($x$ 또는 $y$)에 따라 미리 정해진 특정 각도**로 측정합니다.

    양자역학의 측정 결과는 확률적이지만, 얽힘 때문에 두 사람의 측정 결과는 **고전적으로 불가능한 상관관계**를 갖습니다.

    * **최적의 측정 각도:** 양자역학은 $S = 2\sqrt{2}$를 달성하기 위해 앨리스와 밥이 $\theta_A, \theta_A', \theta_B, \theta_B'$를 $22.5^\circ$ 간격으로 측정하도록 지시합니다. 
        * 예를 들어, $\theta_A$를 $0^\circ$로 잡으면, $\theta_A' = 45^\circ$, $\theta_B = 22.5^\circ$, $\theta_B' = 67.5^\circ$와 같이 설정하는 것이 최적의 전략입니다.

    이 최적의 측정 전략을 사용하면, 네 가지 입력 경우에 대한 성공 확률의 합이 고전적 한계 **2**를 넘어서 **$2\sqrt{2}$**까지 증가합니다.

    2) **승률 계산**

    성공 확률은 벨 매개변수 $S$를 이용하여 계산됩니다.

    $P_{\text{quantum}} = \frac{1}{2} + \frac{S}{8}$

    최적의 양자 상태와 측정 전략을 사용하면 $S$의 최댓값은 **$2\sqrt{2}$ (치렐손 한계)**가 됩니다.

    $P_{\text{quantum}} = \frac{1}{2} + \frac{2\sqrt{2}}{8} = \frac{1}{2} + \frac{\sqrt{2}}{4} \approx 0.5 + 0.3535 = \mathbf{0.8535}$

    4\. **결론**

    CHSH 게임이 고전적 전략보다 뛰어나다는 것은, 양자 얽힘이 **단순히 사전에 합의된 고전적 정보(0.75)**를 넘어서, **숨겨진 상관관계($0.8535$)**를 제공하여 게임의 승률을 끌어올릴 수 있음을 증명합니다. 이는 얽힘이 **고전적인 물리 원칙을 초월하는 자원**임을 보여줍니다.
...
*   **회로 구성**:
    ...
    <img src="https://quantum.cloud.ibm.com/learning/images/courses/fundamentals-of-quantum-algorithms/quantum-query-algorithms/Deutsch-circuit.svg" style="width:60%">
...
*   **회로 구성**: 
    ...
    <img src="https://quantum.cloud.ibm.com/learning/images/courses/fundamentals-of-quantum-algorithms/quantum-query-algorithms/Deutsch-Jozsa.svg" style="width:60%">

*   **응용 사례**:
    *   **Bernstein-Vazirani 문제**: 
        - 숨겨진 문자열 $ s $를 찾는 문제로, Deutsch-Jozsa 회로를 활용해 단 1번의 쿼리로 해결 (고전적 알고리즘은 최소 $ n $번 쿼리 필요).

        <img src="https://qis.rwth-aachen.de/teaching/24ss/intro-quantum-computing/script/bernstein_vazirani.svg" style="width:70%; background:white">
...
*   **회로 구성**:
    ...
    <img src="https://quantum.cloud.ibm.com/learning/images/courses/fundamentals-of-quantum-algorithms/quantum-query-algorithms/Simon.svg" style="width:60%">
...
10. 고전적인 연산을 양자 컴퓨터에서 수행하는 주요 동기는 무엇이며, 이를 위해 토플리 게이트가 어떻게 활용되는가?
>  고전적인 연산을 양자 컴퓨터에서 수행하는 주요 동기는 양자 알고리즘의 서브루틴으로 고전적인 알고리즘을 활용하여 양자 컴퓨팅의 이점을 얻기 위함이다. 토플리 게이트는 AND 연산을 가역적으로 수행할 수 있어, 이를 이용하여 다른 부울 게이트를 시뮬레이션하고 궁극적으로 임의의 고전 연산을 양자 회로로 구현하는 데 핵심적인 역할을 한다.
