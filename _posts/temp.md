...
쇼어 알고리즘(Shor's algorithm)에서 양자 컴퓨터를 통해 주기 을 구한 뒤, 고전적인 컴퓨터를 이용해 인수를 찾아내는 과정을 따라가면 정답을 구할 수 있습니다.
...
2. **이 짝수인지 확인:**
* 으로 짝수입니다. 만약 홀수였다면 알고리즘이 실패했겠지만, 짝수이므로 다음 단계로 진행합니다.


3. ** 계산:**
* 입니다.
* 우리는 $x = a^{r/2} = 3^{120} \pmod{12155}$의 값을 계산해야 합니다.
* 또한 인지 확인해야 하는데, 만약 그렇다면 알고리즘은 실패합니다.


4. **최대공약수(GCD)를 이용한 인수 추출:**
쇼어 알고리즘에서 찾는 두 인수는 다음과 같습니다.
* 
* 
...
실제로 $3^{120} \pmod{12155}$를 계산하면 다음과 같습니다.
(계산 과정을 생략하고 결과를 도출하면)
...
* 
* 
*  ... (유클리드 호제법 진행 시 최종적으로 **17**이 인수로 도출됨)
* 실제 계산: 이므로, **17**은 의 인수입니다.
...
이 결과가 도출되는 과정에서 유클리드 호제법이나 모듈로 거듭제곱 계산에 대해 더 궁금한 점이 있으신가요?