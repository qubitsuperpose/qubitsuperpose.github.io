...
<img src="https://www.researchgate.net/profile/Md-Saifur-Rahman-8/publication/357926217/figure/fig2/AS:1113804398170113@1642563070209/Quantum-circuit-for-Superdense-coding.jpg" style="width:60%">
...
첫 번째 양자 프로토콜인 초고밀도 코딩부터 시작해 보겠습니다. 이 양자 프로토콜은 얽힘 현상을 이용하여 단 하나의 큐비트만 전송하여 2비트의 고전적 정보(00, 01, 10 또는 11)를 전송할 수 있습니다. 이는 고전적 컴퓨터보다 속도가 훨씬 빠릅니다. 고전적 컴퓨터에서는 2비트의 정보를 전송하려면 2비트를 전송해야 하지만, 양자 컴퓨터에서는 단 하나의 큐비트만 전송하면 되기 때문입니다! 
  - 양자 얽힘이 "자원"으로 사용될 수 있음을 보여줌
  - 양자 통신의 이론적 가능성을 보여주는 프로토콜이지, 실제로 정보를 "싸게" 보내는 방법은 아닙니다
...
CNOT와 H는 벨 기저 상태를 표준 기저로 변환(양자 얽힌 상태를 고전 정보로 "디코딩")합니다. 중요한 점은 큐비트 중 하나에만 게이트를 적용하여 상태 $\frac{1}{\sqrt{2}}(\|00\rangle + \|11\rangle)$ 에서 벨 기저의 모든 상태를 생성할 수 있다는 것입니다.
...
> 답: 앨리스는 자신의 큐비트(첫번째)에 X 게이트를 적용하여 다음 상태를 생성합니다 \
...
$$\text{함수 } U_f: (x, c) \rightarrow (x, c \oplus f(x))$$
...
### 2. 양자 컴퓨터에 함수 적용하기 2
비트를 $0$과 $XOR$ 연산하면 같은 비트가 남습니다. $0 \oplus 0 = 1$이고 $0 \oplus 1 = 1$입니다. 따라서 $x \oplus 0 = x$이고, 여기서 $x \in \\{0,1\\}$입니다. 이 사실을 이용하여 $f(x)$의 값을 구할 수 있습니다.
...
$\|\psi_3\rangle = \frac{1}{\sqrt{2}} ( -\|0\rangle + \|1\rangle ) \otimes \|-\rangle = - \frac{1}{\sqrt{2}} ( \|0\rangle - \|1\rangle ) \otimes \|-\rangle \equiv \frac{1}{\sqrt{2}} ( \|0\rangle - \|1\rangle ) \otimes \|-\rangle, \quad \text{if } f(0)=1, f(1)=0$
(전역위상 -1은 무시됨)
